...
def main():
    ...
    if not os.path.isfile(csv_path):
        ...
        alt_path = os.path.join(project_root, 'data', '05_csvs_with_variables', os.path.basename(csv_path))
        if os.path.isfile(alt_path):
            csv_path = alt_path
        else:
            print(f"Error: File '{args.csv}' does not exist in the current directory or in 'data/05_csvs_with_variables' relative to the project root.")
            sys.exit(1)
