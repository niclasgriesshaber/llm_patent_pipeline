...
def match_entries_with_ids(gt_df: pd.DataFrame, llm_df: pd.DataFrame, mode: str, threshold: float = 0.85) -> Tuple[List[bool], List[bool], List[str], List[str]]:
    ...
    if mode == 'strict':
        ...
        for i, gt in enumerate(gt_entries):
            ...
            for j, llm in enumerate(llm_entries):
                score = Levenshtein.normalized_similarity(gt, llm)
                if score > best_score:
                    best_score = score
                    best_idx = j
            if best_idx != -1:
...
def make_intro_html() -> str:
    ...
    <section class="intro-section">
        <div class="intro-box" style="background:#fff; border-radius:8px; max-width:900px; margin:32px auto 24px auto; padding:20px; box-shadow:0 2px 8px rgba(0,0,0,0.07);">
            <p>
                The ground truth dataset was created by student assistants. The LLM output was generated by Gemini-2.0-Flash, which is not yet the best available model (the more advanced Gemini-2.5-Flash is forthcoming). The scaling factor in fuzzy matching allows for some differences between entries and is less strict than exact matching. Fuzzy matching now uses the normalized Levenshtein distance (edit distance) as a similarity measure. The threshold can be set with <code>--scaling</code> (default: 0.85).
            </p>
        </div>
    </section>
    '''
    ...
    <section class="legend-section">
        ...
        <div class="legend-box" style="background:#fff; border-radius:8px; max-width:900px; margin:0 auto 32px auto; padding:24px; box-shadow:0 2px 8px rgba(0,0,0,0.07);">
            ...
            <ul>
                ...
                <li>Fuzzy matching uses normalized Levenshtein similarity (edit distance), which ranges from 0.0 (completely different) to 1.0 (identical). The threshold can be set with <code>--scaling</code> (default: 0.85).</li>
            </ul>
        </div>
    </section>
    '''
...
def make_fuzzy_year_chart(metrics_by_year: List[Dict], threshold: float) -> str:
    ...
    <section class="chart-section">
        <h2>Fuzzy Matching: Match Rate by Year (Threshold {threshold})</h2>
...
def make_fuzzy_scaling_chart(metrics_by_threshold: List[Tuple[float, float]]) -> str:
    ...
    axes = f'<line x1="{margin}" y1="{height - margin}" x2="{width - margin}" y2="{height - margin}" stroke="#333" stroke-width="2" />' \
           f'<line x1="{margin}" y1="{height - margin}" x2="{margin}" y2="{margin}" stroke="#333" stroke-width="2" />'
    xlabel = f'<text x="{width/2}" y="{height - 10}" text-anchor="middle" font-size="14">Threshold</text>'
    ylabel = f'<text x="20" y="{height/2}" text-anchor="middle" font-size="14" transform="rotate(-90 20,{height/2})">Match Rate (%)</text>'
    ...
    svg = f'''
    <section class="chart-section">
        <h2>Fuzzy Matching: Match Rate vs. Threshold</h2>
        <svg width="{width}" height="{height}" style="background:#fff; border-radius:8px; box-shadow:0 2px 8px rgba(0,0,0,0.07); display:block; margin:0 auto;">
            {axes}
            {curve}
            {dots}
...
def main():
    ...
    parser.add_argument('--scaling', type=float, default=0.85, help='Threshold for normalized Levenshtein similarity in fuzzy matching (default: 0.85)')
    ...
    fuzzy_year_chart = make_fuzzy_year_chart(fuzzy_metrics_by_year, threshold=scaling)
    # Fuzzy scaling chart
    thresholds = [round(x * 0.1, 2) for x in range(0, 11)]
